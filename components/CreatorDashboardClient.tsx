...
function CreatorDashboard({ data }: { data: CreatorDashboardData }) {
  ...
  return (
    <>
      <header className="workspaceTopbar creatorTopbar"><div><span>кабинет Саундмейкера · {profile.name}</span><h1>Ваша страница вкуса</h1></div><button type="button" className={`pauseButton ${paused ? "resume" : ""}`} disabled={busy === "pause" || busy === "resume"} onClick={() => void togglePaused()}><Icon name={paused ? "play" : "pause"} />{busy === "pause" || busy === "resume" ? "Сохраняем…" : paused ? "Возобновить публикацию" : "Поставить на паузу"}</button></header>
