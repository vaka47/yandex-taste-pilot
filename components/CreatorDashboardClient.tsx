...
function CreatorDashboard({ data }: { data: CreatorDashboardData }) {
  ...
  return (
    <>
      <header className="workspaceTopbar creatorTopbar"><div><span>кабинет тейстмейкера · {profile.name}</span><h1>Ваша страница вкуса</h1></div><button type="button" className={`pauseButton ${paused ? "resume" : ""}`} disabled={busy === "pause" || busy === "resume"} onClick={() => { const resume = paused; setPaused(!paused); void saveControl(resume ? "resume" : "pause", undefined, resume ? "Публикация возобновлена." : "Публикация приостановлена."); }}><Icon name={paused ? "play" : "pause"} />{paused ? "Возобновить публикацию" : "Поставить на паузу"}</button></header>
