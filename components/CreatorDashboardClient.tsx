...
type ConfirmAction = "disconnect" | "delete_request" | null;
...
const syncOptions = [
  ...
  { value: 300, label: "Раз в 5 минут" },
  { value: 900, label: "Раз в 15 минут" },
...
export function CreatorDashboardClient({ initialData }: { initialData: CreatorDashboardData | null }) {
  if (!initialData) return <section className="workspaceEmpty"><Icon name="users" /><h1>Профиль ещё не назначен</h1><p>Откройте персональную ссылку-приглашение, которую прислал владелец Тейста.</p></section>;
...
function CreatorDashboard({ data }: { data: CreatorDashboardData }) {
  ...
  async function confirmDestructiveAction() {
    ...
    if (!action) return;
    const saved = await saveControl(action, undefined, action === "disconnect" ? "Яндекс Музыка отключена." : "Запрос на удаление принят.");
  ...
  return (
    <>
      <header className="workspaceTopbar creatorTopbar"><div><span>кабинет тейстмейкера · {profile.name}</span><h1>Ваша страница вкуса</h1></div><button type="button" className={`pauseButton ${paused ? "resume" : ""}`} disabled={busy === "pause" || busy === "resume"} onClick={() => void togglePaused()}><Icon name={paused ? "play" : "pause"} />{busy === "pause" || busy === "resume" ? "Сохраняем…" : paused ? "Возобновить публикацию" : "Поставить на паузу"}</button></header>
      ...
      <section className="creatorPanel historyPanel" id="history"><header><div><span>ваша история</span><h2>Последние события</h2></div><div className="historyLegend"><span><i className="public" />опубликовано</span><span><i className="scheduled" />ожидает</span><span><i className="hidden" />скрыто</span></div></header><div className="creatorEventList">{data.events.length ? data.events.map(event => { const isHidden = hidden.includes(event.id); const hiddenByArtist = isHidden && event.track.artists.some(name => blockedArtists.some(item => item.name.trim().toLowerCase() === name.trim().toLowerCase())); return <article key={event.id} className={isHidden ? "eventHidden" : ""}><CoverArt tone={event.track.coverTone} title={event.track.title} size="small" /><div><strong>{event.track.title}</strong><span>{event.track.artists.join(", ")}</span></div><time>{listeningTime(event)}</time><span className={`visibilityBadge ${isHidden ? "hidden" : event.visibility}`}><i />{hiddenByArtist ? "скрыт правилом" : isHidden ? "скрыт автором" : visibilityLabels[event.visibility]}</span><button type="button" disabled={hiddenByArtist || busy === "hide_event" || busy === "restore_event"} onClick={() => void hideEvent(event.id)}>{hiddenByArtist ? "Правило выше" : isHidden ? "Вернуть" : "Скрыть трек"}</button><button type="button" className="artistHide" disabled={busy === "hide_artist" || hiddenByArtist || !event.track.artists[0]} onClick={() => void hideArtist(event.track.artists[0])}>{hiddenByArtist ? "Исполнитель скрыт" : "Скрыть исполнителя"}</button></article>; }) : <article><div><strong>История пока пуста</strong><span>После подключения первая запись из истории Яндекс Музыки появится здесь автоматически.</span></div></article>}</div></section>

      <section className="creatorGrid lowerCreatorGrid"><article className="creatorPanel playlistPanel"><header><div><span>плейлист в аккаунте followtaste</span><h2>Живой плейлист</h2></div><Icon name="playlist" /></header><div><span className="playlistCover"><i /><b>ТЕЙСТ</b><small>{profile.name}</small></span><div><strong>Вкус {profile.name} — живой</strong><p>Последние {data.playlist.maxTracks} уникальных разрешённых треков. Повторы считаются на вашей странице, но не дублируются в плейлисте.</p>{data.playlist.url ? <a href={data.playlist.url} target="_blank" rel="noreferrer">Открыть в Яндекс Музыке <Icon name="arrow" /></a> : <span>Плейлист создастся автоматически после первой записи в истории.</span>}</div></div><footer><span>{data.playlist.trackCount} треков · версия {data.playlist.revision ?? "—"}</span><em>Обновляется автоматически</em></footer></article><article className="creatorPanel consentPanel"><header><div><span>согласие и данные</span><h2>Ваш контроль</h2></div><Icon name="check" /></header><dl><div><dt>Версия согласия</dt><dd>{data.consentVersion || "не подтверждено"}</dd></div><div><dt>Подтверждено</dt><dd>{data.consentAt ? new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "long", year: "numeric" }).format(new Date(data.consentAt)) : "—"}</dd></div><div><dt>Состояние</dt><dd>{data.connection.status === "connected" ? "активно" : "ожидает подключения"}</dd></div></dl><button type="button" disabled={data.connection.status !== "connected"} onClick={() => setConfirmAction("disconnect")}>Отключить Яндекс Музыку</button><button type="button" className="dangerLink" onClick={() => setConfirmAction("delete_request")}>Запросить удаление данных</button></article></section>

      {confirmAction ? <div className="modalBackdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target && !busy) setConfirmAction(null); }}><section className="workspaceModal confirmModal" role="alertdialog" aria-modal="true" aria-labelledby="confirm-action-title"><button type="button" disabled={Boolean(busy)} onClick={() => setConfirmAction(null)} aria-label="Закрыть"><Icon name="x" /></button><span className="confirmModalIcon"><Icon name="shield" size={28} /></span><span>действие требует подтверждения</span><h2 id="confirm-action-title">{confirmAction === "disconnect" ? "Отключить историю?" : "Запросить удаление данных?"}</h2><p>{confirmAction === "disconnect" ? "Тейст перестанет получать новые записи и обновлять плейлист. Уже опубликованные треки останутся видимыми, пока владелец не удалит их отдельно." : "Владелец получит запрос и свяжется с вами для подтверждения состава удаляемых данных. До завершения процедуры профиль не исчезнет автоматически."}</p><div><button type="button" className="ghostButton" disabled={Boolean(busy)} onClick={() => setConfirmAction(null)}>Отмена</button><button type="button" className="dangerButton" disabled={Boolean(busy)} onClick={() => void confirmDestructiveAction()}>{busy ? "Выполняем…" : confirmAction === "disconnect" ? "Да, отключить" : "Отправить запрос"}</button></div></section></div> : null}
