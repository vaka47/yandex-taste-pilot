  ...
  return (
    <>
      ...
      <section className="creatorGrid">
        <article className="creatorPanel connectionPanel" id="connection"><header><div><span>connection 01 · source</span><h2>Личная история</h2></div><span className={`statusTag status-${data.connection.status === "connected" ? "active" : data.connection.status}`}><i />{data.connection.status}</span></header><div className="accountCard"><span className="yandexAvatar">Я</span><div><strong>{data.connection.login || "Личный аккаунт не подключён"}</strong><small>{data.connection.accountIdSuffix ? `Источник истории · ID •••• ${data.connection.accountIdSuffix}` : "Подключите основной аккаунт, где вы слушаете музыку"}</small></div><button type="button" onClick={() => setConnectOpen(true)}>{data.connection.status === "connected" ? "Переподключить" : "Подключить историю"}</button></div><dl><div><dt>Последняя синхронизация</dt><dd>{data.connection.lastSuccessAt ? relativeTime(data.connection.lastSuccessAt) : "ещё не было"}</dd></div><div><dt>Срок токена</dt><dd>{expiresLabel(data.connection.expiresAt)}</dd></div><div><dt>Последняя ошибка</dt><dd>{data.connection.errorCode || "нет"}</dd></div><div><dt>Интервал</dt><dd>5 минут</dd></div></dl><footer><span><Icon name="lock" />Токен зашифрован и никогда не показывается в браузере.</span><button type="button" disabled={data.connection.status !== "connected"} onClick={() => void saveControl("sync_now")}><Icon name="sync" />Синхронизировать</button></footer></article>
      ...
      <section className="creatorGrid lowerCreatorGrid"><article className="creatorPanel playlistPanel"><header><div><span>delivery 04 · followtaste</span><h2>Живой плейлист</h2></div><Icon name="playlist" /></header><div><span className="playlistCover"><i /><b>TASTE</b><small>{data.name}</small></span><div><strong>Вкус {data.name} — live</strong><p>Публикует followtaste: последние {data.playlist.maxTracks} уникальных разрешённых треков, один стабильный URL.</p>{data.playlist.url ? <a href={data.playlist.url} target="_blank" rel="noreferrer">Открыть в Яндекс Музыке <Icon name="arrow" /></a> : <span>Плейлист появится в followtaste после первой успешной синхронизации.</span>}</div></div><footer><span>{data.playlist.trackCount} треков · revision {data.playlist.revision ?? "—"}</span><button type="button" disabled={data.connection.status !== "connected"} onClick={() => void saveControl("playlist_sync")}><Icon name="sync" />Обновить сейчас</button></footer></article><article className="creatorPanel consentPanel"><header><div><span>consent 05</span><h2>Согласие и данные</h2></div><Icon name="check" /></header><dl><div><dt>Версия согласия</dt><dd>{data.consentVersion || "не подтверждено"}</dd></div><div><dt>Подтверждено</dt><dd>{data.consentAt ? new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "long", year: "numeric" }).format(new Date(data.consentAt)) : "—"}</dd></div><div><dt>Состояние</dt><dd>{data.connection.status === "connected" ? "активно" : "ожидает подключения"}</dd></div></dl><button type="button" disabled={data.connection.status !== "connected"} onClick={() => void saveControl("disconnect")}>Отключить Яндекс Музыку</button><button type="button" className="dangerLink" onClick={() => void saveControl("delete_request")}>Запросить удаление данных</button></article></section>

      {connectOpen ? <div className="modalBackdrop"><section className="workspaceModal deviceModal"><button type="button" onClick={() => { setConnectOpen(false); setChallenge(null); }}><Icon name="x" /></button><span>источник истории · личный аккаунт</span><h2>Подключить то, что вы слушаете</h2>{!challenge ? <><p>Подключите основной аккаунт Яндекс Музыки — тот, где вы реально слушаете треки. Не используйте здесь <b>followtaste</b>: он только создаёт итоговые плейлисты.</p><div className="consentChecklist"><span><Icon name="check" />История читается только из этого аккаунта</span><span><Icon name="check" />Разрешённые треки попадут в live-плейлист</span><span><Icon name="check" />Доступ можно отключить в любой момент</span></div><button className="darkButton wideButton" type="button" onClick={() => void startConnection()}>{connectionState === "starting" ? "Получаем код…" : "Получить код для личного аккаунта"}</button></> : <><p>На странице Яндекса переключитесь на ваш основной музыкальный аккаунт, введите код и подтвердите доступ. Проверьте логин перед подтверждением: это не должен быть followtaste.</p><div className="deviceCode"><small>код личного аккаунта</small><strong>{challenge.userCode}</strong><button type="button" onClick={() => void navigator.clipboard.writeText(challenge.userCode)}><Icon name="copy" />Копировать</button></div><a className="darkButton wideButton" href={challenge.verificationUrl} target="_blank" rel="noreferrer">Открыть страницу Яндекса <Icon name="arrow" /></a><div className="waitingState"><i /><span>Ждём подтверждения основного аккаунта. Это окно можно оставить открытым.</span></div></>}</section></div> : null}
    </>
  );
}
