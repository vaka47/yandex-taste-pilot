  ...
  return (
    <header className={landing ? "landingHeader" : "publicHeader"}>
      ...
      <div className={`headerActions ${landing ? "landingHeaderActions" : ""}`}>
        ...
        {landing ? session ? <Link className="headerAccountLink" href="/following"><Icon name="user" size={16} />Мои подписки</Link> : <Link className="headerAccountLink" href="/auth/yandex/start?returnTo=/" prefetch={false}><Icon name="user" size={16} />Войти</Link> : session ? <form action="/auth/logout" method="post"><button className="headerLogin" type="submit"><Icon name="user" size={17} /> Выйти</button></form> : <Link className="headerLogin" href="/auth/yandex/start?returnTo=/" prefetch={false}><Icon name="user" size={17} /> Войти</Link>}
