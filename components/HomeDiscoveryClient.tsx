    ...
    <>
      <section className="landingHero">
        <div className="landingCopy">
          ...
          <p>Живая история прослушиваний артистов, авторов и людей с собственным вкусом.</p>
          <div className="landingHeroActions"><a className="landingPrimary" href="#discover"><Icon name="search" />Найти автора</a><Link className="landingSecondary" href="/about">Как это устроено <Icon name="arrow" /></Link></div>
        </div>
        <div className="landingVisual" onPointerEnter={() => setPaused(true)} onPointerLeave={() => setPaused(false)} onPointerDown={event => { pointerStart.current = event.clientX; }} onPointerUp={event => endSwipe(event.clientX)}>
          {current ? <Link className="landingSlide" href={`/t/${current.slug}`} aria-label={`Открыть профиль ${current.name}`}>
            ...
            <div className="landingSignalCard"><small>{current.updatedAt ? `обновлено ${relativeTime(current.updatedAt)}` : "история подключена"}</small><strong>{current.name}</strong><span>{current.latestTrack ? `${current.latestTrack.title} · ${current.latestTrack.artists.join(", ")}` : current.roleLine}</span><i>▂▅▃▇▆▂▅</i></div>
          </Link> : <div className="landingSlide landingSlideEmpty"><div className="landingSlidePortrait"><Icon name="music" size={52} /></div><div className="landingSignalCard"><strong>Taste</strong><span>Скоро здесь появятся первые авторы</span></div></div>}
          {profiles.length > 1 ? <div className="landingCarouselControls"><button type="button" onClick={() => shift(-1)} aria-label="Предыдущий автор"><Icon name="arrow" /></button><span>{profiles.map((profile, index) => <button key={profile.id} className={index === active ? "active" : ""} type="button" onClick={() => setActive(index)} aria-label={`Показать ${profile.name}`} />)}</span><button type="button" onClick={() => shift(1)} aria-label="Следующий автор"><Icon name="arrow" /></button></div> : null}
        </div>
      </section>

      <section className="landingManifest"><strong>НЕ РЕКОМЕНДАЦИИ, А ЧЕЛОВЕЧЕСКИЙ ОПЫТ</strong></section>

      <section className="landingDiscover" id="discover">
        <header><div><span>люди в Taste</span><h2>Найдите того,<br />кого хотите слышать.</h2></div><label><Icon name="search" /><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Имя автора" aria-label="Поиск по авторам" /></label></header>
        <div className="landingPeople">{found.map(profile => <Link href={`/t/${profile.slug}`} key={profile.id}><span>{profile.avatarUrl ? <img src={profile.avatarUrl} alt="" loading="lazy" /> : profile.name.split(" ").map(word => word[0]).join("").slice(0, 2)}</span><div><strong>{profile.name}</strong><small>{profile.roleLine || "живая история музыки"}</small>{profile.latestTrack ? <em>Сейчас в истории: {profile.latestTrack.title}</em> : null}</div><Icon name="arrow" /></Link>)}{!found.length ? <div className="landingNoResults"><Icon name="search" /><strong>Такого автора пока нет</strong><span>Проверьте имя или вернитесь к списку.</span></div> : null}</div>
      </section>

      <section className="landingActivity">
        <header><span>прямо сейчас</span><h2>Что нового в Taste</h2></header>
        <div>{activity.length ? activity.map((item, index) => <Link href={item.kind === "comment" ? `/go/track/${item.eventId}?source=home_comment&position=${index + 1}` : `/t/${item.tastemakerSlug}`} target={item.kind === "comment" ? "_blank" : undefined} rel={item.kind === "comment" ? "noreferrer" : undefined} key={item.id}><span>{String(index + 1).padStart(2, "0")}</span><p>{item.kind === "comment" ? <><strong>{item.tastemakerName}</strong> — о треке «{item.trackTitle}»: <em>«{item.comment}»</em></> : <><strong>{item.tastemakerName}</strong> послушал «{item.trackTitle}»</>}</p><time>{relativeTime(item.occurredAt)}</time><Icon name="arrow" /></Link>) : <p className="landingActivityEmpty">Первые обновления появятся после подключения истории автора.</p>}</div>
      </section>
    </>
  );
}
