...
function auditLabel(action: string) {
  const labels: Record<string, string> = {
    tastemaker_created: "Саундмейкер добавлен", creator_invite_claimed: "Приглашение принято", creator_invite_created: "Выпущено новое приглашение",
    ...
    admin_tastemaker_intro_updated: "Представление Саундмейкера обновлено", creator_onboarding_seen: "Саундмейкер прошёл знакомство с кабинетом"
  };
...
export function AdminDashboardClient({ initialData }: { initialData: AdminDashboardData }) {
  ...
  const [introDrafts, setIntroDrafts] = useState<Record<string, string>>(() => Object.fromEntries(initialData.tastemakers.map(item => [item.id, item.roleLine])));
  const [search, setSearch] = useState("");
  const [pendingDelete, setPendingDelete] = useState<AdminTastemakerRow | null>(null);
  ...
  const filteredTastemakers = tastemakers.filter(item => `${item.name} ${item.slug} ${item.roleLine}`.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  async function action(type: string, tastemakerId?: string, payload: Partial<NewTastemaker> & { confirmName?: string } = {}) {
    ...
    if (response.ok && result.inviteUrl) {
      if (type === "create_tastemaker" && result.id) {
        ...
        setIntroDrafts(values => ({ ...values, [result.id!]: roleLine }));
        setInviteOpen(false);
    ...
    if (response.ok && type === "update_tastemaker_intro" && tastemakerId) {
      const roleLine = result.roleLine || payload.roleLine?.trim() || "";
      setTastemakers(items => items.map(item => item.id === tastemakerId ? { ...item, roleLine } : item));
      setIntroDrafts(values => ({ ...values, [tastemakerId]: roleLine }));
      setNotice({ tone: "success", text: "Представление Саундмейкера обновлено на главной странице." });
      return;
    }
    ...
    if (response.ok && type === "delete_tastemaker" && tastemakerId) {
      setTastemakers(items => items.filter(item => item.id !== tastemakerId));
      setPendingDelete(null);
      setDeleteConfirmation("");
      setNotice({ tone: "success", text: "Профиль Саундмейкера и связанные с ним данные удалены." });
      return;
    }
    const errorText = result.error === "PLAYLIST_DELETE_FAILED" ? "Страница Саундмейкера уже скрыта, но плейлист не удалось удалить из Яндекс Музыки. Проверьте followtaste и повторите удаление." : result.error === "ACTION_FAILED" ? "Не удалось выполнить действие. Возможно, такой адрес страницы уже занят." : result.error === "NOT_FOUND" && type === "create_invite" ? "Профиль уже зарегистрирован: повторный инвайт для него закрыт." : "Действие не выполнено. Проверьте состояние подключения.";
  ...
  return (
    <>
      ...
      <section className="adminPanel makersPanel" id="tastemakers">
        <header><div><span>аналитика по каждому Саундмейкеру</span><h2>Подключённые Саундмейкеры</h2></div><label className="adminSearch"><Icon name="search" size={18} /><input type="search" value={search} onChange={event => setSearch(event.target.value)} placeholder="Найти Саундмейкера" aria-label="Найти Саундмейкера" /></label></header>
        <div className="adminMakerGrid">{filteredTastemakers.map(item => {
          const musicActions = item.trackOpens7d + item.playlistOpens7d;
          const funnelBase = Math.max(item.uniqueVisitors7d, 1);
          const introDraft = introDrafts[item.id] ?? item.roleLine;
          return <details className="adminMakerCard" key={item.id}>
            ...
            <div className="makerIntroEditor"><div><span>представление на главной</span><p><strong>{item.name}</strong> — {item.roleLine}</p></div><label htmlFor={`intro-${item.id}`}>Кто это<input id={`intro-${item.id}`} value={introDraft} maxLength={120} onChange={event => setIntroDrafts(values => ({ ...values, [item.id]: event.target.value }))} placeholder="популярный стендап-комик" /></label><button type="button" disabled={introDraft.trim().length < 2 || introDraft.trim() === item.roleLine || busy === `update_tastemaker_intro:${item.id}`} onClick={() => void action("update_tastemaker_intro", item.id, { roleLine: introDraft })}>{busy === `update_tastemaker_intro:${item.id}` ? "Сохраняем…" : "Сохранить"}</button></div>
