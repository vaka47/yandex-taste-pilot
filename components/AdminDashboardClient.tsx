...
export function AdminDashboardClient({ initialData }: { initialData: AdminDashboardData }) {
  ...
  async function action(type: string, tastemakerId?: string, payload: Partial<NewTastemaker> & { confirmName?: string } = {}) {
    ...
    if (response.ok && result.inviteUrl) {
      if (type === "create_tastemaker" && result.id) {
        setTastemakers(items => [{ id: result.id!, slug: result.slug || payload.slug || "new-soundmaker", name: payload.name || "Новый Саундмейкер", gender: payload.gender || "neutral", avatarUrl: null, status: "invited", registered: false, followerCount: 0, profileViews7d: 0, uniqueVisitors7d: 0, historyUnlocks7d: 0, authCompletions7d: 0, followClicks7d: 0, follows7d: 0, trackOpens7d: 0, playlistOpens7d: 0, shares7d: 0, telegramSubscribers: 0, telegramClicks7d: 0, returnVisitors7d: 0, d1Retention: 0, d7Retention: 0, followerD7Retention: 0, lastSyncAt: null, playlistUrl: null, playlistStatus: "not_created", connectionStatus: "not_connected" }, ...items]);
        setInviteOpen(false);
  ...
  return (
    <>
      ...
      {inviteOpen ? <div className="modalBackdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) setInviteOpen(false); }}><section className="workspaceModal"><button type="button" onClick={() => setInviteOpen(false)} aria-label="Закрыть"><Icon name="x" /></button><span>новый Саундмейкер</span><h2>Пригласить Саундмейкера</h2><label>Имя<input value={draftMaker.name} maxLength={100} onChange={event => setDraftMaker(value => ({ ...value, name: event.target.value }))} placeholder="Например: Иван Сафонов" /></label><label>Адрес страницы<input value={draftMaker.slug} maxLength={60} onChange={event => setDraftMaker(value => ({ ...value, slug: event.target.value }))} placeholder="ivan-safonov" /></label><label>Короткая подпись<input value={draftMaker.roleLine} maxLength={120} onChange={event => setDraftMaker(value => ({ ...value, roleLine: event.target.value }))} placeholder="музыкант · режиссёр" /></label><label>Форма обращения<select value={draftMaker.gender} onChange={event => setDraftMaker(value => ({ ...value, gender: event.target.value as TastemakerGender }))}><option value="male">Мужчина — послушал</option><option value="female">Женщина — послушала</option><option value="neutral">Без указания — слушает</option></select></label><p>Выберите форму один раз: она нужна только для естественных фраз в ленте. Ссылка действует 7 дней и сгорает после регистрации.</p><div><button type="button" className="ghostButton" onClick={() => setInviteOpen(false)}>Отмена</button><button type="button" className="darkButton" disabled={!draftMaker.name.trim() || !draftMaker.slug.trim() || busy === "create_tastemaker:global"} onClick={() => void action("create_tastemaker", undefined, draftMaker)}>{busy === "create_tastemaker:global" ? "Создаём…" : "Создать и скопировать ссылку"}</button></div></section></div> : null}
