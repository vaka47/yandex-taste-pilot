  ...
  async function action(type: string, tastemakerId?: string, payload: Partial<NewTastemaker> = {}) {
    ...
    if (response.ok && result.inviteUrl) {
      if (type === "create_tastemaker" && result.id) {
        setTastemakers(items => [{ id: result.id!, slug: result.slug || payload.slug || "new-author", name: payload.name || "Новый автор", avatarUrl: null, status: "invited", registered: false, followerCount: 0, profileViews7d: 0, uniqueVisitors7d: 0, followClicks7d: 0, follows7d: 0, trackOpens7d: 0, playlistOpens7d: 0, shares7d: 0, returnVisitors7d: 0, lastSyncAt: null, playlistUrl: null, playlistStatus: "not_created", connectionStatus: "not_connected" }, ...items]);
        setInviteOpen(false);
    ...
    setNotice(response.ok ? { tone: "success", text: type === "sync" ? "История проверена, плейлист обновлён." : "Изменение применено." } : { tone: "warning", text: errorText });
  }
  ...
  return (
    <>
      ...
      <section className="adminPanel makersPanel" id="tastemakers">
        ...
        <div className="adminMakerGrid">{tastemakers.map(item => {
          ...
          return <details className="adminMakerCard" key={item.id}>
            ...
            <div className="makerAnalytics"><div><span>Все просмотры страницы</span><strong>{fullNumber(item.profileViews7d)}</strong></div><div><span>Вернулись в другой день</span><strong>{fullNumber(item.returnVisitors7d)}</strong><small>{percent(item.returnVisitors7d, item.uniqueVisitors7d)} от посетителей</small></div><div><span>Открыли трек</span><strong>{fullNumber(item.trackOpens7d)}</strong></div><div><span>Открыли плейлист</span><strong>{fullNumber(item.playlistOpens7d)}</strong></div><div><span>Поделились страницей</span><strong>{fullNumber(item.shares7d)}</strong></div></div>
      ...
      <section className="adminGrid" id="sync">
        ...
        <article className="adminPanel automationPanel"><header><div><span>как работает публикация</span><h2>Без ручных действий</h2></div><Icon name="sync" /></header><ol><li><b>1</b><span>Тейст регулярно проверяет историю подключённых авторов и дополнительно обновляется при открытии страницы.</span></li><li><b>2</b><span>Мы берём записи, которые Яндекс уже показал в истории. Точный порог прослушивания Яндекс не раскрывает.</span></li><li><b>3</b><span>Без задержки новая запись сразу появляется на странице и в плейлисте аккаунта followtaste.</span></li></ol><p>По умолчанию выбран самый быстрый режим. Автор может настроить частоту проверки и задержку публикации в своём кабинете.</p></article>
      </section>
