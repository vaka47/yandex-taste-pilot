...
function auditLabel(action: string) {
  const labels: Record<string, string> = {
    tastemaker_created: "Автор создан", creator_invite_claimed: "Приглашение принято", creator_invite_created: "Выпущено новое приглашение",
...
export function AdminDashboardClient({ initialData }: { initialData: AdminDashboardData }) {
  ...
  const filteredTastemakers = tastemakers.filter(item => `${item.name} ${item.slug}`.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return (
    <>
      <header className="workspaceTopbar"><div><span>кабинет владельца · последние 7 дней</span><h1>Управление авторами</h1></div><div><a className="ghostButton" href="/api/admin/export?kind=daily"><Icon name="arrow" />Выгрузить аналитику</a><button type="button" className="darkButton" onClick={() => setInviteOpen(true)}><Icon name="users" />Пригласить автора</button></div></header>
      ...
      <section className="adminPanel makersPanel" id="tastemakers">
        <header><div><span>аналитика по каждому автору</span><h2>Подключённые авторы</h2></div><label className="adminSearch"><Icon name="search" size={18} /><input type="search" value={search} onChange={event => setSearch(event.target.value)} placeholder="Найти автора" aria-label="Найти автора" /></label></header>
        <div className="adminMakerGrid">{filteredTastemakers.map(item => {
          ...
          return <details className="adminMakerCard" key={item.id}>
            ...
            <footer><div><span>История: <b>{connectionLabels[item.connectionStatus]}</b></span><span>Плейлист: <b>{playlistLabels[item.playlistStatus]}</b></span><span>Обновление: <b>{item.lastSyncAt ? relativeTime(item.lastSyncAt) : "ещё не было"}</b></span></div><nav><a href={`/t/${item.slug}`} target="_blank" rel="noreferrer"><Icon name="eye" />Открыть страницу</a><a href={`/api/admin/export?kind=daily&tastemakerId=${item.id}`}><Icon name="arrow" />Выгрузить данные</a>{item.connectionStatus === "connected" ? <button type="button" disabled={busy === `sync:${item.id}`} onClick={() => void action("sync", item.id)}><Icon name="sync" />{busy === `sync:${item.id}` ? "Обновляем…" : "Обновить сейчас"}</button> : null}{!item.registered ? <button type="button" disabled={busy === `create_invite:${item.id}`} onClick={() => void action("create_invite", item.id)}><Icon name="copy" />Новая ссылка</button> : null}{["active", "paused"].includes(item.status) ? <button type="button" disabled={busy === `pause:${item.id}`} onClick={() => void action("pause", item.id)}><Icon name={item.status === "paused" ? "play" : "pause"} />{item.status === "paused" ? "Возобновить" : "На паузу"}</button> : null}<button type="button" className="dangerTextButton" onClick={() => { setPendingDelete(item); setDeleteConfirmation(""); }}><Icon name="x" />Удалить автора</button></nav></footer>
          </details>;
      ...
      {inviteOpen ? <div className="modalBackdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) setInviteOpen(false); }}><section className="workspaceModal"><button type="button" onClick={() => setInviteOpen(false)} aria-label="Закрыть"><Icon name="x" /></button><span>новый автор</span><h2>Пригласить автора</h2><label>Имя<input value={draftMaker.name} maxLength={100} onChange={event => setDraftMaker(value => ({ ...value, name: event.target.value }))} placeholder="Как будет показано публично" /></label><label>Адрес страницы<input value={draftMaker.slug} maxLength={60} onChange={event => setDraftMaker(value => ({ ...value, slug: event.target.value }))} placeholder="ivan-petrov" /></label><label>Короткая подпись<input value={draftMaker.roleLine} maxLength={120} onChange={event => setDraftMaker(value => ({ ...value, roleLine: event.target.value }))} placeholder="музыкант · режиссёр" /></label><p>Ссылка действует 7 дней и сгорает сразу после регистрации этого автора.</p><div><button type="button" className="ghostButton" onClick={() => setInviteOpen(false)}>Отмена</button><button type="button" className="darkButton" disabled={!draftMaker.name.trim() || !draftMaker.slug.trim() || busy === "create_tastemaker:global"} onClick={() => void action("create_tastemaker", undefined, draftMaker)}>{busy === "create_tastemaker:global" ? "Создаём…" : "Создать и скопировать ссылку"}</button></div></section></div> : null}
      {pendingDelete ? <div className="modalBackdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target && !busy) setPendingDelete(null); }}><section className="workspaceModal confirmModal" role="alertdialog" aria-modal="true" aria-labelledby="delete-tastemaker-title"><button type="button" disabled={Boolean(busy)} onClick={() => setPendingDelete(null)} aria-label="Закрыть"><Icon name="x" /></button><span className="confirmModalIcon"><Icon name="shield" size={28} /></span><span>необратимое действие</span><h2 id="delete-tastemaker-title">Удалить {pendingDelete.name}?</h2><p>Профиль, история, подписки, комментарии и аналитика этого автора будут удалены. Для подтверждения введите имя полностью.</p><label>Имя автора<input autoFocus value={deleteConfirmation} onChange={event => setDeleteConfirmation(event.target.value)} placeholder={pendingDelete.name} /></label><div><button type="button" className="ghostButton" disabled={Boolean(busy)} onClick={() => setPendingDelete(null)}>Отмена</button><button type="button" className="dangerButton" disabled={deleteConfirmation.trim() !== pendingDelete.name || busy === `delete_tastemaker:${pendingDelete.id}`} onClick={() => void action("delete_tastemaker", pendingDelete.id, { confirmName: deleteConfirmation })}>{busy === `delete_tastemaker:${pendingDelete.id}` ? "Удаляем…" : "Удалить навсегда"}</button></div></section></div> : null}
