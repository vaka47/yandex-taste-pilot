...
Production automation runs on the Yandex Cloud VM: the lightweight Compose scheduler calls the protected sync route once a minute, while a local systemd watchdog recovers unhealthy containers every two minutes. Active public profiles and the creator cabinet can also request a safe post-response sync. The GitHub workflows remain available only as manual break-glass actions; their schedules are disabled so the retired Vercel database cannot be mutated in parallel and no duplicate automation is billed. This is polling rather than a real-time Yandex webhook, so publication follows the first successful poll after Yandex exposes the history event.

The backup container creates one PostgreSQL custom-format dump per day, validates it with `pg_restore --list`, retains seven local copies and removes interrupted `.tmp` files. In Yandex Cloud production it encrypts the verified dump with AES-256 before uploading it to the private `nitca-production-backups` Object Storage bucket using the VM service account's short-lived IAM token. The recovery key is stored separately in the owner's macOS Keychain, no static S3 key exists, and the bucket lifecycle removes encrypted remote dumps after 30 days.

For Telegram digests, create a bot in BotFather and set `TELEGRAM_NOTIFICATIONS_ENABLED=true`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME` and a random `TELEGRAM_WEBHOOK_SECRET`. The current pilot uses the Vercel deployment as a free network bridge: Telegram posts to its webhook, the exact body is HMAC-signed with the bot token and forwarded to `https://nitca.ru/api/telegram/webhook`; outbound Bot API calls use the same authenticated bridge. The bridge never opens the rollback database. A fan must first sign in with Yandex ID and follow the tastemaker; Telegram linking is voluntary and per tastemaker. `/stop` disables all bot notifications. A single subscription uses the 20:00 Moscow slot; multiple subscriptions are spread into distinct, stable slots from 12:00 to 21:00 so one listener does not receive a burst. The system never sends more than one digest per tastemaker per Moscow calendar day, and only after that tastemaker's public playlist has caught up with the new stored history. Creator comments bypass the digest schedule and are delivered immediately.
