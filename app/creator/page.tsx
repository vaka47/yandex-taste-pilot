...
export default async function CreatorPage({ searchParams }: { searchParams: Promise<{ preview?: string }> }) {
  ...
  return <WorkspaceShell area="creator" preview={isPreview} profileHref={data ? `/t/${data.slug}` : "/t/lera-sever"}><CreatorDashboardClient preview={isPreview} initialData={data} /></WorkspaceShell>;
}
