...
export default async function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="invitePage"><Brand /><section><span><Icon name="spark" /> личное приглашение в Taste</span><h1>Ваш музыкальный вкус уже ждут.</h1><p>Войдите через Яндекс ID — после принятия приглашения сразу откроется ваш кабинет и следующий шаг подключения музыкальной истории.</p><Link href={href} prefetch={false}><b>Я</b>Принять приглашение <Icon name="arrow" /></Link><small>Ссылка одноразовая и имеет срок действия. Доступ к истории вы подтвердите отдельно в Яндексе.</small></section></main>;
}
