...
export default async function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="invitePage"><Brand /><section><span><Icon name="spark" /> приглашение в пилот</span><h1>Ваш музыкальный вкус уже ждут.</h1><p>Войдите через Yandex ID, чтобы привязать кабинет автора. Подключение музыкальной истории произойдёт отдельным шагом и только после явного согласия.</p><Link href={href}><b>Я</b>Принять приглашение <Icon name="arrow" /></Link><small>Ссылка одноразовая и имеет срок действия.</small></section></main>;
}
