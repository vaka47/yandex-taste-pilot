...
export default async function HomePage() {
  ...
  return (
    <main className="landingPage">
      <header className="landingHeader"><Brand /><nav><Link href={liveHref}>{featured ? "Живой профиль" : "Пилот готовится"}</Link><Link href="/about">Как работает</Link><Link href="/privacy">Приватность</Link></nav><Link className="landingHeaderCta" href={liveHref}>{featured ? "Открыть Тейст" : "О пилоте"} <Icon name="arrow" /></Link></header>
      <section className="landingHero">
        <div className="landingCopy"><span className="landingKicker"><i /> независимый музыкальный пилот</span><h1>Слышать мир<br /><em>чужими ушами.</em></h1><p>Реальная, добровольно опубликованная история прослушиваний людей, чьему вкусу вы доверяете.</p><div><Link className="landingPrimary" href={liveHref}><Icon name="pulse" />{featured ? "Смотреть живой Тейст" : "Узнать о пилоте"}</Link><Link className="landingSecondary" href="/about">Как это устроено <Icon name="arrow" /></Link></div></div>
        <div className="landingVisual"><span className="landingEdition">пилот / 001</span><ProfilePortrait name={featured?.name} avatarUrl={featured?.avatarUrl} /><div className="landingSignalCard"><small>{signal ? "сейчас в Тейсте" : "контур готов"}</small><strong>{signal?.track.title || featured?.name || "Ждём первый сигнал"}</strong><span>{signal?.track.artists.join(", ") || (featured ? "История обновляется" : "Автор ещё не подключён")}</span><i>▂▅▃▇▆▂▅</i></div><div className="landingOrbit">вкус · вживую · с разрешения ·</div></div>
      </section>
      ...
      <section className="landingFlow"><header><span>01—03</span><h2>Один новый трек.<br />Три честных шага.</h2></header><div className="landingSteps"><article><b>01</b><Icon name="music" /><h3>Кумир слушает</h3><p>Только после явного согласия запись из истории попадает в защищённый контур Тейста.</p></article><article><b>02</b><Icon name="pulse" /><h3>Тейст показывает</h3><p>С учётом задержки, паузы и скрытых треков. Приватность управляется самим автором.</p></article><article><b>03</b><Icon name="playlist" /><h3>Фанат открывает</h3><p>Трек или один постоянный живой плейлист — уже внутри Яндекс Музыки.</p></article></div></section>
      ...
      <footer className="landingFooter"><Brand inverse /><p>Независимый экспериментальный продукт. Не связан с Яндексом и не одобрен им.</p><nav><Link href="/privacy">Приватность</Link><Link href="/creator">Кабинет тейстмейкера</Link></nav></footer>
    </main>
  );
}
