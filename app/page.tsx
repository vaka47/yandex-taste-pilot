...
export default async function HomePage() {
  ...
  return (
    <main className="landingPage">
      <header className="landingHeader"><Brand /><nav><Link href={liveHref}>{featured ? "Живой профиль" : "Пилот готовится"}</Link><Link href="/about">Как работает</Link><Link href="/admin?preview=1">Админка</Link></nav><Link className="landingHeaderCta" href={liveHref}>{featured ? "Открыть Taste" : "О пилоте"} <Icon name="arrow" /></Link></header>
      <section className="landingHero">
        ...
        <div className="landingVisual"><span className="landingEdition">pilot / 001</span><ProfilePortrait name={featured?.name} /><div className="landingSignalCard"><small>{signal ? "сейчас в Taste" : "контур готов"}</small><strong>{signal?.track.title || featured?.name || "Ждём первый сигнал"}</strong><span>{signal?.track.artists.join(", ") || (featured ? "История синхронизируется" : "Автор ещё не подключён")}</span><i>▂▅▃▇▆▂▅</i></div><div className="landingOrbit">вкус · вживую · с разрешения ·</div></div>
      </section>
